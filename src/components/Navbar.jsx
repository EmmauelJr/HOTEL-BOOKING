...
const Navbar = () => {
    ...
    return (
            <nav className={`fixed top-0 left-0 w-full flex items-center justify-between 
                ...
                {/* Logo */}
                <Link to='/'>
                    <h1 className={`h-12 text-4xl font-merriweather text-white ${isScrolled && "invert opacity-80"}`}>
                        Velora Suites
                    </h1>
                </Link>
                ...
                <div className="flex items-center gap-3 md:hidden">
                    <img onClick={() => setIsMenuOpen(!isMenuOpen)} src={assets.menuIcon} alt="menu" 
                    className={`${isScrolled && "invert"} h-4`} />
                </div>

                {/* Mobile Menu */}
                <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col 
